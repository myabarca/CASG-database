...
if (isset($_POST['search_button'])) { 
        ...
        // note that empty(0) returns true (0 is seen as "empty"), so empty won't work for fields where 0 is an acceptable value. php just thinks it's empty. so have to combine isset & < 0 (eg no negative numbers) to check that locs and lots with ID's of 0 can be passed into search    

        if (empty($_POST['genus_search']) && empty($_POST['species_search']) && empty($_POST['taxon_search']) && empty($_POST['continent_search']) &&empty($_POST['country_search']) && empty($_POST['state_search']) && empty($_POST['county_search']) && empty($_POST['eon_search']) && empty($_POST['era_search']) && empty($_POST['period_search']) && empty($_POST['epoch_search']) && ($_POST['minlat_search'] != "") && ($_POST['maxlat_search'] != "") && ($_POST['minlong_search'] != "") && ($_POST['maxlong_search'] != "") && empty($_POST['age_search']) && empty($_POST['stage_search']) && empty($_POST['stratgrp_search']) && empty($_POST['stratform_search']) && empty($_POST['stratmem_search']) && isset($_POST['locationID_search']) && $_POST['locationID_search'] < 0 && isset($_POST['lotID_search']) && $_POST['lotID_search'] < 0 && isset($_POST['previousColln_search']) && $_POST['previousColln_search'] < 0 && empty($_POST['collector_search'])) {

            echo 'Please enter valid data in fields above to search.' ;

        // elseif here becomes the sql query is different (one more join to goereference table) when people search for coordinates    
        } elseif (($_POST['minlat_search']=="") && ($_POST['minlong_search']=="") && ($_POST['maxlong_search']=="") && ($_POST['maxlat_search']=="")) {

            $sql = 'SELECT lot.tier, lot.drawerID, lot.lotID, lot.genus, lot.species, locality.locationID, locality.country, locality.stateProvince, locality.county, 
            ...
            // including all taxon name related columns in this search part      
            array_push($sql_arr, ' lot.scientificName LIKE :entered_taxon OR lot.genus LIKE :entered_taxon OR lot.species LIKE :entered_taxon OR lot.family LIKE :entered_taxon');
            // array_push($bind_arr, "'entered_species' => %$searched_species%");    
            }
            ...
            array_push($sql_arr, ' locality.earliestAgeOrLowestStage LIKE :entered_stage OR locality.latestAgeOrLowestStage LIKE :entered_stage');
            ...
            array_push($sql_arr, ' lot.prevColln LIKE :entered_previousColln');
            // array_push($bind_arr, "'entered_locationID' => %$searched_locationID%");
            }
            ...
            $sql = 'SELECT lot.tier, lot.drawerID, lot.lotID, lot.genus, lot.species, locality.locationID, locality.country, locality.stateProvince, locality.county, locality.earliestEraOrLowestErathem, locality.earliestPeriodOrLowestSystem, locality.earliestEpochOrLowestSeries 
            ...
            if (!empty($_POST['county_search'])) {
                ...
                // CANNOT USE EMPTY HERE since 0 is a valid coordinate value
                // use does not equal empty string instead        
            if ($_POST['minlat_search']!= "") {
                $searched_minlat = $_POST['minlat_search'];

                array_push($sql_arr, ' georeference.decimalLatitude BETWEEN :entered_minlat AND :entered_maxlat');
            }

            if ($_POST['maxlat_search']!= "") {
                $searched_maxlat = $_POST['maxlat_search'];
                        
                array_push($sql_arr, ' georeference.decimalLatitude BETWEEN :entered_minlat AND :entered_maxlat');
            }

            if ($_POST['minlong_search']!= "") {
                $searched_minlong = $_POST['minlong_search'];
                        
                array_push($sql_arr, ' georeference.decimalLongitude BETWEEN :entered_minlong AND :entered_maxlong');
            }

            if ($_POST['maxlong_search']!= "") {
                $searched_maxlong = $_POST['maxlong_search'];
            ...
            array_push($sql_arr, ' locality.earliestAgeOrLowestStage OR locality.latestAgeOrHighestStage LIKE :entered_stage');
            ...
            if ($_POST['minlat_search'] != "") {
            $stmt->bindValue(':entered_minlat', $searched_minlat, PDO::PARAM_STR);
            }

            if ($_POST['maxlat_search'] != "") {
            $stmt->bindValue(':entered_maxlat', $searched_maxlat, PDO::PARAM_STR);
            }

            if ($_POST['minlong_search'] != "") {
            $stmt->bindValue(':entered_minlong', $searched_minlong, PDO::PARAM_STR);
            }

            if ($_POST['maxlong_search'] != "") {
            $stmt->bindValue(':entered_maxlong', $searched_maxlong, PDO::PARAM_STR);
            }
